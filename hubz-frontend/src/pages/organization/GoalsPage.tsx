...
export default function GoalsPage() {
  ...
  const handleCreate = async (data: CreateGoalRequest) => {
    if (!orgId) return;
    try {
      await goalService.createOrganizationGoal(orgId, data);
  ...
  return (
    <div className="flex h-full flex-col gap-4 overflow-auto p-4 sm:gap-6 sm:p-6">
      {/* Header */}
      <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        <div>
          <h2 className="text-xl font-bold text-gray-900 dark:text-gray-100 sm:text-2xl">Objectifs</h2>
          <p className="mt-1 hidden text-sm text-gray-500 dark:text-gray-400 sm:block">
            Gerez les objectifs de votre organisation. La progression se calcule automatiquement en fonction des taches associees.
          </p>
        </div>
